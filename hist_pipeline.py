...
class Pipeline():
    ...
    def start_pipeline(self):
        ...
        self.aligned_histology.volumize(self.final_out +'/hist_to_bf.nii.gz')
    ...
    def preprocess_histology(self):
        ...
        self.histology_color.zpad(self.orig_hist_loc + '/orig/',self.overwrite) #Zero-pad color images
...
#####################################################################################################################        
    def blockface_to_MRI_alignment(self,out_dir):
        ...
        if self.overwrite == False and os.path.isfile(out_dir + 'MRI_to_blockface_linear_alignment.nii.gz'):
            ...
            reg.inputs.number_of_iterations = ([[1500,1000,500]] * 2)
    ...
    def colorize(self,col_out_dir,final_out_dir):
        ...
        if skip_flag == False:
            print('====================================ATTEMPTING TO MULTITHREAD====================================')
            pool = Pool(processes=self.threads)
            pool.map(Transform_Wrapper(self.hist_NIFTI,self.hist_transform,self.BF_NIFTI,self.orig_slice_by_slice_loc +'/color/'), list(range(len(self.hist_transform.slices))))
            pool.close()
            pool.join()
    ...
    def final_apply_transform(self,in_file,out_file):
        ...
        at.inputs.reference_image = self.MRI_path + os.path.split(self.orig_MRI)[1]
...
class Register_Wrapper(object):
    ...
    def register(self,i):
        ...
        if self.reg_method == 'nonlinear':
            ...
            reg.inputs.number_of_iterations = ([[1500,1000,500]] * 4)
            ...
            reg.inputs.number_of_iterations = ([[1500,1000,500]] * 3)
...
class Transform_Wrapper(object):
    ...
    """
    def __init__(self, hist_NIFTI,hist_transform,BF_NIFTI,out_dir):
        """
        Defines static inputs for all iterations of the transformation.
        """
        self.hist_NIFTI = hist_NIFTI
    ...
    def __call__(self, i):
        ...
        """
        out_suf_list = ['Blue','Green','Red']
        for j, col_vol in enumerate([self.hist_NIFTI.Blue_vol, self.hist_NIFTI.Green_vol, self.hist_NIFTI.Red_vol]):
            self.apply_transform_slice(i,col_vol,out_suf_list[j])
